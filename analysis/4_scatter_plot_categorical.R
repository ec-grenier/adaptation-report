...
incbin_scatter_plot = function(sector, category, scn, gwl_bin, period, popwt=F, bins=100, zoom=NULL, slug=''){
  
  ylab = "Deaths per 100,000"
  xlab = ifelse(popwt, "Population weighted income bins (countries ranked by GDPpc)", "Income bins (countries ranked by GDPpc)")
  ...
  # get income
  gdppc = get_dscim_econ_vars(c('gdppc', 'pop'), 'iso', 'low', 'SSP2', 2025) %>% ungroup() %>% select(region, pop, gdppc)

  # plot data
  impacts = rates %>% left_join(levels) %>% left_join(gdppc) %>% filter(!is.na(gdppc))
  
  if (!popwt){
    impacts = impacts %>% mutate(gdppc_bin = ntile(gdppc, bins))
  } else {
    impacts = impacts %>% arrange(gdppc) %>% 
      mutate(cum_pop = cumsum(pop) / sum(pop),
             gdppc_bin = cut(cum_pop,
                             breaks = seq(0, 1, length.out = bins + 1), # e.g. 10 bins => 0,0.1,...,1
                             labels = 1:bins,
                             include.lowest = TRUE))
    slug='-popwt'
  }
  ...
  impacts = impacts %>%
    mutate(gdppc_bin = factor(gdppc_bin)) %>%
    mutate(type = ifelse(levels.q50 >= 0, "Lives lost", "Lives saved"),
           size_val = abs(levels.q50))
  
  p = ggplot(impacts, aes(x = gdppc_bin, y = rates.q50)) +
    ...
    scale_x_discrete(breaks = as.character(seq(5, bins, by = 5))) +
    labs(x = xlab,
         y = ylab,
         title = plot_title,
         subtitle=subtitle) +
  ...
  print(p)
  message(glue('[Saving: ] country_scatter_categorical-{scn}-{category}-{period}-{gwl_bin}{slug}.jpg'))
  ggsave(glue('{output}/{sector}/country_scatter_categorical-{scn}-{category}-{period}-{gwl_bin}{slug}.jpg'), p, height=5, width=8)
  message(" ---- DONE ---- \n ")
}
...
for (age in c('combined', 'oldest', 'older', 'young')){
  incbin_scatter_plot(sector = "mortality",
                      category = age,
                      scn = "fulladapt",
                      gwl_bin = "3_c",
                      period = "midc",
