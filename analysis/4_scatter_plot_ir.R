...
ir_scatter_plot = function(sector, category, scn, gwl_bin, period, zoom=NULL, slug=''){
  ...
  rates = read.csv(glue("{input}/{sector}/{category}-{scn}-ir_level-rates-{gwl_bin}-{period}-SSP2.csv")) %>%
    select(region, q50) %>% 
    rename(rates.q50 = q50)

  # get income
  gdppc = get_dscim_econ_vars('gdppc', 'iso', 'low', 'SSP2', 2025) %>% ungroup() %>% select(region, gdppc) %>% rename(iso=region)

  # plot data
  impacts = rates %>% left_join(levels) 
  impacts = impacts %>% mutate(iso = substr(region,1,3))
  impacts = impacts %>% left_join(gdppc) %>% filter(!is.na(gdppc))
  iso_ranks = impacts %>% distinct(iso, gdppc) %>% arrange(gdppc) %>% mutate(iso_index = row_number())
  impacts = impacts %>% left_join(iso_ranks)
  ...
  p = ggplot(impacts, aes(x = iso_index, y = rates.q50)) +
  ...
  ggsave(glue('{output}/{sector}/ir_rates_scatter-{scn}-{category}-{period}-{gwl_bin}{slug}.jpg'), p, height=5, width=8)
  message(" ---- DONE ---- \n ")
}
