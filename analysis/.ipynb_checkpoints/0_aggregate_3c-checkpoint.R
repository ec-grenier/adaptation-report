...
raw_gwl_data_a = read.csv(glue('{input}/{sector}/rcp45-SSP2-{gwl_bin}_{period}_a-no_surrogate-{scn}-{category}-low-tails-{spatial}{allyears}-{unit}-valuescsv.csv'))
raw_gwl_data_b = read.csv(glue('{input}/{sector}/rcp85-SSP2-{gwl_bin}_{period}_b-no_surrogate-{scn}-{category}-low-tails-{spatial}{allyears}-{unit}-valuescsv.csv'))
...
write.csv(raw_gwl_data, 
          glue('/project/cil/home_dirs/egrenier/cil-comms/adaptation_report/data/analysis_ready/{sector}/{category}-{scn}-{spatial}-{unit}-{gwl_bin}-{period}-SSP2-low-tails{allyears}.csv'), 
          row.names=F)
