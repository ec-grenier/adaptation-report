...
title = glue("Mid century {sector} combined age group rankings (3C warming)")
...
p = ggplot(data = plot.df) +
  ...
  scale_fill_gradientn(colors = palette, 
                       ...
                       guide = guide_colorbar(title = glue("Impact regions ranked by damages ({ntiles} groups, low to high)"), 
                                              ...
                                              label.hjust = 0.5))
print(p)
ggsave(glue("/project/cil/home_dirs/egrenier/cil-comms/adaptation_report/output/IR_ranked-{sector}-{scn}-{category}-{period}-{gwl_bin}-{ntiles}_bins.pdf"), p)
