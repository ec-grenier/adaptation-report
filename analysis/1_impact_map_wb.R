...
spatial = 'ir_level'
slug=''
exclude = c("high") # high or upper-middle or both
...
print(impact.map)
message(glue("Saving: {output}/{sector}/{prefix}-{scn}-{category}-{period}-{gwl_bin}-{spatial}-{unit}{slug}.pdf"))
ggsave(glue("{output}/{sector}/{prefix}-{scn}-{category}-{period}-{gwl_bin}-{spatial}-{unit}{slug}.pdf"), impact.map)
