...
CONFIG=${REPO}/inequality/bash_scripts/inequality_extract_ag_config.yml
...
results_root="/project/cil/gcp/outputs/agriculture/agval/adaptation_report_costs" #path to Monte carlos to be used
        ...
        for rcp in ${rcplist[@]}; do
    		for ssp in ${ssplist[@]}; do
    			for iam in ${iamlist[@]}; do
					...
					valcol="--column=wc_no_reallocation"
					post="none"
                	outsuffix="-${bin}-no_surrogate-${scn}-${iam}"
