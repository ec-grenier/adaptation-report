...
#region_list = list('PNG.8.38')

slug=''

#-----set args here-----
args = list(years=year,
            base_year=c(1981:2005),
            rebase_year=c(2005,2001:2004,2006:2010),
            bound_to_hist=F,
            csvv.dir=csvv_dir,
            csvv.name=csvv_name,
            het.list=het_list,
...
for (age in age_list){
  ...
  # Load in climate data for all years (1981 to 2010 + the impact year)
  extraction_years = sort(unique(c(args$years, args$base_year, args$rebase_year)))
  clim_df = mapply_extract_climate_data(years=extraction_years, tas_value=args$tas_value, ncname=args$ncname, rcp=rcp, gcm=gcm)
  
  df = lapply(region_list, get_all_db_tables, age, args, clim_df)
  df = do.call(rbind, df) %>% as.data.frame() %>% select(region, bin, effect_fa)
  
  file_name = glue("mortality-delta_beta-fulladapt-{year}-{agegroup}-avg.csv")
